...
namespace GiaoDien
{
    public partial class CreateRoles : Form
    {
       ...
       // }
        ...
        private void buttonApply_Click(object sender, EventArgs e)
        {
            if (txtRoleName.Text == "" )
            {
                this.Alert_GrantUser("Please input the rolename", Form_Alert.enmType.Failed);
            ...
            try
            {
                ...
                if (txtNewPass.Text != "")
                {
                    sql = "create ROLE " + txtRoleName.Text.ToUpper() + " identified by " + txtNewPass.Text;
                    ...
                    foreach (DataGridViewRow r in dgvRole.Rows)
                    {
                        ...
                        if (Convert.ToBoolean(r.Cells[1].Value) == true || Convert.ToBoolean(r.Cells[0].Value) == true)
                        {
                            if (Convert.ToBoolean(r.Cells[0].Value) == true)
                                ...
                                sql = "GRANT \"" + r.Cells[2].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" WITH ADMIN OPTION ";
                            else
                                sql = "GRANT \"" + r.Cells[2].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" ";
                            command.CommandText = sql;
                            command.ExecuteNonQuery();
                        }
                    }
                    ...
                    foreach (DataGridViewRow r1 in dgvSysPri.Rows)
                    {
                        if (Convert.ToBoolean(r1.Cells[1].Value) == true || Convert.ToBoolean(r1.Cells[0].Value) == true)
                        {
                            if (Convert.ToBoolean(r1.Cells[0].Value) == true)
                                sql = "GRANT " + r1.Cells[2].Value.ToString() + " TO \"" + txtRoleName.Text.ToUpper() + "\" WITH ADMIN OPTION ";
                            else
                                sql = "GRANT " + r1.Cells[2].Value.ToString() + " TO \"" + txtRoleName.Text.ToUpper() + "\" ";
                            command.CommandText = sql;
                            command.ExecuteNonQuery();
                        }
                    }
                    ...
                    foreach (DataGridViewRow r2 in dgvTable.Rows)
                    {
                        if (Convert.ToBoolean(r2.Cells[0].Value) == true)
                        {
                            if (Convert.ToBoolean(r2.Cells[5].Value) == true)
                                sql = "GRANT ALL ON \"" + r2.Cells[6].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" WITH GRANT OPTION ";
                            else
                                sql = "GRANT ALL ON \"" + r2.Cells[6].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" ";
                            command.CommandText = sql;
                            command.ExecuteNonQuery();
                        }
                        else
                        {
                            ...
                            {
                                if (Convert.ToBoolean(r2.Cells[5].Value) == true)
                                    sql = "GRANT SELECT ON \"" + r2.Cells[6].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" WITH GRANT OPTION ";
                                else
                                    sql = "GRANT SELECT ON \"" + r2.Cells[6].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" ";
                                command.CommandText = sql;
                                command.ExecuteNonQuery();
                            }
                            ...
                            {
                                if (Convert.ToBoolean(r2.Cells[5].Value) == true)
                                    sql = "GRANT INSERT ON \"" + r2.Cells[6].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" WITH GRANT OPTION ";
                                else
                                    sql = "GRANT INSERT ON \"" + r2.Cells[6].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" ";
                                command.CommandText = sql;
                                command.ExecuteNonQuery();
                            }
                            ...
                            {
                                if (Convert.ToBoolean(r2.Cells[5].Value) == true)
                                    sql = "GRANT UPDATE ON \"" + r2.Cells[6].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" WITH GRANT OPTION ";
                                else
                                    sql = "GRANT UPDATE ON \"" + r2.Cells[6].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" ";
                                command.CommandText = sql;
                                command.ExecuteNonQuery();
                            }
                            ...
                            {
                                if (Convert.ToBoolean(r2.Cells[5].Value) == true)
                                    sql = "GRANT DELETE ON \"" + r2.Cells[6].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" WITH GRANT OPTION ";
                                else
                                    sql = "GRANT DELETE ON \"" + r2.Cells[6].Value.ToString() + "\" TO \"" + txtRoleName.Text.ToUpper() + "\" ";
                                command.CommandText = sql;
                                command.ExecuteNonQuery();
                            }
                        }
                ...
                {
                    sql = "create ROLE " + txtRoleName.Text.ToUpper();
                    command.CommandText = sql;
                    command.ExecuteNonQuery();
                }
        ...
        private void buttonGrantUpdate_Click(object sender, EventArgs e)
        {
            ...
            sql = sql + ") ON " + txtTable.Text + " TO \"" + txtRoleName.Text + "\"";
            listupdate.Add(sql);
        ...
        private void dgvTable_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                txtTable.Enabled = false;
                //load dgvCol
                using (OracleConnection orcCont1 = new OracleConnection(KetNoi.AdminConnect))
                {
        ...
        { // Tạo view với cấu trúc tên: VIEW_[tên bảng]_[tên user/role]
            string sql;
            string nameview = "VIEW_"+txtTable.Text+"_"+txtRoleName.Text;
            bool fi = false; // biến kt  có dùng dấu phẩy giữa các thuộc tính k)
            ...
            string grant = "GRANT SELECT ON " + nameview + " TO " + txtRoleName.Text;
            listview.Add(grant);
